<template>
  <t-query-panel :model="query" size="medium">
    ...
    <el-table class="tw-table" :data="table.data" border height="100%" stripe>
      ...
      <el-table-column prop="CNQX" label="权限" show-overflow-tooltip></el-table-column>
...
import { data } from '../../assets/js/menu.config.json'
...
export default {
  ...
  data() {
    return {
      ...
      data: [],
      menuLists: JSON.parse(JSON.stringify(data)),
      username: ''
    }
  },
  mounted() {
  ...
  methods: {
    ...
    submitQx() {
      ...
      if (this.dialog.title == '添加权限') {
        ajaxT.post('user/addPower', params, { baseURL }).then(res => {
          this.dialog.visible = false
          if (res.data == '1') {
            this.$message({
              message: '添加成功',
      ...
      if (this.dialog.title == '修改权限') {
        ajaxT.post('user/updatePower', params, { baseURL }).then(res => {
          this.dialog.visible = false
          if (res.data == '1') {
            this.$message({
              message: '添加成功',
