  ...
  <t-query-panel :model="query" size="medium">
    ...
    <template>
      ...
      >
        ...
        <el-table-column
          align="center"
          prop="TRAFFICVIOLATIONCOUNT"
          label="交通违章次数"
          width="160"
        ></el-table-column>
        <el-table-column
          align="center"
          prop="COMPLAINEDCOUNT"
          label="被投诉次数"
          width="160"
        ></el-table-column>
        <el-table-column
  ...
  methods: {
    ...
    getDataList() {
      ...
      if (stime == '' || etime == '' || !stime || !etime) {
        ...
        let idata = `postData={'ptname':'${this.query.ptname}','jszh':'${
          this.query.jszh
        }','endtime':'${moment(this.query.etime).format('YYYY-MM-DD HH:mm:ss')}','begintime':'${moment(
          this.query.stime
        ).format('YYYY-MM-DD HH:mm:ss')}','pageIndex':'${this.table.currentPage}','pageSize':'${
          this.table.pageSize
        }' }`
        ajaxT.post('manage/findJsytjxx', idata, { baseURL: this.baseURL }).then(res => {
          let data = res.data
