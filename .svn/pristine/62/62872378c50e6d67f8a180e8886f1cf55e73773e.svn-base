...
export default {
  ...
  methods: {
    loginClick() {
      ...
      ajaxT.post('user/login', params).then(res => {
        if (res.data === '登陆成功') {
          ...
          ajaxT.post('user/getUserPower', qxParams).then(res => {
            ...
            if (res.data.length == 0) {
              ...
              this.routerList = []
              if (res.data[0].QX.indexOf('yingyunzonglan') > -1)
                this.$router.push({ path: '/yingyunzonglan' })
              else {
                this.formateHRouterList(qxStr[0], JSON.parse(JSON.stringify(getMenu())))
                if (this.routerList.length === 0)
                  this.formateHRouterList(qxStr[1], JSON.parse(JSON.stringify(getMenu())))
