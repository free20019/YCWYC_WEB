...
export default {
  ...
  computed: {
    filterSystemMenu() {
      ...
      return _.filter(menuList, item => this.filterSecondaryMenu(item.id).length > 0)
  ...
  methods: {
    ...
    filterSecondaryMenuList(menuList) {
      ...
      _.each(menuList, item => {
        if (item.children) newMenuList.push(...this.filterSecondaryMenuList(item.children))
        else if (item.title.indexOf(this.query) >= 0 || this.query.length === 0) newMenuList.push(item)
      })
      return newMenuList
    },
    ...
    handleTotalMenuGroupItemClick(item) {
      ...
      const systemMenu = _.filter(this.systemMenu, item => item.id === systemId)[0]
    ...
    handleTotalSystemMenuClick(item) {
      ...
      let total = _.filter(jump, _item => _item.getAttribute('index') === item.id)
