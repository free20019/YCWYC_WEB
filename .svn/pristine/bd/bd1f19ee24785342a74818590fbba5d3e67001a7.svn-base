...
<template>
  <t-query-panel :model="query" size="medium">
    ...
    <el-table stripe :data="table.data" border height="100%">
  ...
  data() {
    let checkUser = (rule, value, callback) => {
      ...
      if (!value) {
        ...
        ajaxT.post('user/checkUsername', params, { baseURL }).then(res => {
          if (res.data == '1') {
    ...
    let validatePassEasy = (rule, value, callback) => {
      let isDigit = /^.*[0-9]+.*/
      let isLowerCase = /^.*[a-z]+.*/
      let isUpperCase = /^.*[A-Z]+.*/
      let regEx = /^.*[^a-zA-Z0-9]+.*/
    ...
    return {
      ...
      rules: {
        ...
        password: [
          ...
          { validator: validatePassEasy, trigger: 'blur' }
        ],
        phone: []
      },
      selectRow: {},
  ...
  methods: {
    handleTableDel(row) {
      ...
      this.$confirm('确定删除吗?', '提示', {
        ...
        ajaxT.post('user/deleteUser', params, { baseURL }).then(res => {
          this.queryTable()
        })
      })
    },
      ...
      this.$refs.ruleForm.validate(valid => {
        if (valid) {
          ...
          if (this.dialog.title == '添加') {
            ajaxT.post('user/addUser', params, { baseURL }).then(res => {
              this.handleClose()
              this.queryTable()
              this.loadFlag = false
            })
          ...
          if (this.dialog.title == '修改') {
            ...
            ajaxT.post('user/updateUser', params, { baseURL }).then(res => {
              this.handleClose()
              this.queryTable()
              this.loadFlag = false
            })
